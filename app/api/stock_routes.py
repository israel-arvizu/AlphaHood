...
def search_stocks():
    ...
    search_stock_name = Stock.query.filter(Stock.name.ilike(f'{search}%')).all()
    search_ticker = Stock.query.filter(Stock.ticker.ilike(f'{search}%')).all()

    combined = list(set(search_stock_name + search_ticker))

    sorted_combined = sorted(combined, key=lambda k: k.name)

    return {'search': [stocks.to_dict() for stocks in sorted_combined]}
